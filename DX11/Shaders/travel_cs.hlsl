...
RWStructuredBuffer < STravelDataData > TravelData : register(u2);
...
inline void AddMatrix( inout ParticleFrame target, float3 rotation, float3 origin )
{
	...
	target.trans[3][0] = origin.x;
	target.trans[3][1] = origin.y;
	target.trans[3][2] = origin.z;
}

inline void ProcessParticle( uint index, uint localIndex )
{
	TravelData[ localIndex ].position += TravelData[ localIndex ].direction * DT;

	DataToProcFrame[ index ].trans[3].xyz += TravelData[ localIndex ].position;
}
